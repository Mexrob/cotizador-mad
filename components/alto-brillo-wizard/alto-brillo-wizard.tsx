...
interface AltoBrilloWizardProps {
  quoteId: string
...
export default function AltoBrilloWizard({
  quoteId,
  initialData,
  ...
  // ------------------------------------------
  // CARGAR PRODUCTOS DE ALTO BRILLO DESDE LA BASE DE DATOS
  // ------------------------------------------
  
  const loadProducts = useCallback(async () => {
    setLoadingProducts(true)
    try {
      const response = await fetch('/api/products?search=Alto%20Brillo&limit=100')
      const data = await response.json()
      if (data.products && Array.isArray(data.products)) {
        setProducts(data.products)
      }
    } catch (error) {
      console.error('Error loading Alto Brillo products:', error)
    } finally {
  ...
  useEffect(() => {
    console.log('AltoBrillo initialData changed:', initialData)
    if (initialData && Object.keys(initialData).length > 0 && initialData.productId) {
      ...
      console.log('AltoBrillo restoring - tone from data:', initialData.tone, 'found:', toneOption?.name)
      console.log('AltoBrillo isTwoSided from data:', initialData.isTwoSided)
      setState({
        step: 1,
        tone: toneOption?.name || initialData.tone || TONE_OPTIONS[0]?.name || '',
        toneId: initialData.productId || initialData.toneId || '',
      ...
      console.log('AltoBrillo resetting to default state')
      setState({
        step: 1,
  ...
  const canGoNext = () => {
    switch (state.step) {
      case 1: return !!state.toneId || !!state.tone
  ...
  const handleFinish = () => {
    ...
    console.log('AltoBrillo handleFinish:', {
      tone: state.tone,
    ...
    onComplete({
      ...
      tone: state.tone,
      toneId: '',
      productId: state.toneId,
      pricePerSquareMeter: state.tonePrice,
      isTwoSided: state.options.faces === '2',
  ...
  const renderToneStep = () => {
    ...
    return (
      <WizardStepContainer title="Selección de Producto" icon={Palette}>
        {products.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-gray-500">No hay productos disponibles para Alto Brillo</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {products.map((product) => (
              <WizardOptionCard
                key={product.id}
                selected={state.toneId === product.id}
                onClick={() => setState(prev => ({ 
                  ...prev, 
                  tone: product.name,
                  toneId: product.id,
                  tonePrice: product.precioBaseM2 || 0 
                }))}
              >
                <div className="text-center">
                  <h4 className="font-semibold text-sm">{product.name}</h4>
                  <p className="text-lg font-bold text-green-600">
                    ${product.precioBaseM2?.toLocaleString('es-MX') || 0}/m²
                  </p>
                  {product.tiempoEntrega && (
                    <p className="text-xs text-gray-500 mt-1">
                      Entrega: {product.tiempoEntrega} días
                    </p>
                  )}
                </div>
              </WizardOptionCard>
